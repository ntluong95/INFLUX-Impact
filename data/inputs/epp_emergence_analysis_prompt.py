...
Event sheet format: List the major emergence events of the EPP following the criteria for each type of emergence event listed below. Emergence event can be defined as either a single new occurrence or a temporally bounded and geographically/epidemiologically linked episode. Only document events from 1990 onwards, but you should search for earlier events to confirm if the EPP is truly present or absent in the area in the past to better classify an emergence event type.

If there are less than 10, list all events. If there are more than 10, list the 10 most major events in terms of impact (e.g. number of cases and deaths, production loss, or number countries affected). Priority to include emergence events after 1990: First/global emergence > Inter-continental emergence > Intra-continental emergence > Re-emergence > Re-occurrence > First detection
Events should be merged only when they represent continuous or recurrent occurrence for at least two consecutive years in the same outbreak area or in neighboring countries within the same UNSD sub-region. Events should not be merged across sub-regions and continents. Intra-continential, and inter-continental emergence events should ALWAYS be described as concrete single occurrence events if they occur in different years, noting the first year of this concrete episode. For periods characterized by linked episodes, identify the years with the largest outbreaks and the concrete countries where these took place.
...
•	Year (always include the year the event starts, optionally include year the event ends)
...
•	Countries: All countries linked to the event
 Please refer to sheet “Country” in the input file for the detail of continent, subregions and countries.
