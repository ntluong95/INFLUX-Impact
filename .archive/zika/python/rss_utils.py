...
def truncate_for_log(value: str | None, width: int = 200) -> str:
...
def load_config(config_path: Path) -> dict[str, Any]:
    ...
    if chunk_env := getenv_nonempty("RSS_CHUNK_SIZE"):
        rss_cfg["chunk_size_days"] = int(chunk_env)
    else:
    ...
    if lang_env := getenv_nonempty("GOOGLE_NEWS_LANG"):
        rss_cfg["lang"] = lang_env
    if country_env := getenv_nonempty("GOOGLE_NEWS_COUNTRY"):
        rss_cfg["country"] = country_env
    if start_env := getenv_nonempty("ZIKA_QUERY_START_DATE"):
        query_cfg["start_date"] = start_env
    if end_env := getenv_nonempty("ZIKA_QUERY_END_DATE"):
        query_cfg["end_date"] = end_env

    query_cfg["language"] = query_cfg.get("language", rss_cfg.get("lang", "en"))
    return cfg
...
def write_manifest(manifest_df: pd.DataFrame, manifest_csv: Path) -> None:
    write_dataframe(manifest_df[MANIFEST_COLUMNS].fillna(""), manifest_csv, None)
