...
GOOGLE_NEWS_ARTICLE_RE = re.compile(
    r"/(?:rss/)?articles/([^/?#]+)", flags=re.IGNORECASE
)
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("zika_fulltext")
...
def load_config(config_path: Path) -> dict[str, Any]:
    ...
    fulltext = cfg.setdefault("fulltext", {})
    fulltext["rate_limit_per_second"] = float(
        fulltext.get("rate_limit_per_second", 0.5)
        if not (rate_env := os.getenv("FULLTEXT_RATE_LIMIT"))
        else rate_env
    )
    if timeout_env := os.getenv("TIMEOUT_SECONDS"):
        fulltext["timeout_seconds"] = int(timeout_env)
    return cfg
...
def decode_google_news_url_online(
    wrapper_url: str, article_id: str, timeout_seconds: int, session: requests.Session
) -> tuple[str | None, str]:
    try:
        wrapper_resp = session.get(
            wrapper_url, timeout=timeout_seconds, allow_redirects=True
        )
...
def resolve_google_news_url(
    wrapper_url: str, timeout_seconds: int, session: requests.Session
) -> tuple[str, str, str]:
    ...
    online, err = decode_google_news_url_online(
        wrapper_url, article_id, timeout_seconds, session
    )
    if online:
...
def polite_fetch(
    session: requests.Session, url: str, cfg: dict[str, Any], logger: logging.Logger
) -> requests.Response:
...
def extract_metadata(
    html: str, final_url: str, response: requests.Response | None = None
) -> dict[str, Any]:
    ...
    page_title = (
        extract_meta_content(
            soup, [("property", "og:title"), ("name", "twitter:title")]
        )
    ...
    language = (
        ...
        or extract_meta_content(
            soup, [("http-equiv", "content-language"), ("property", "og:locale")]
        )
...
def extract_full_text(html: str, final_url: str) -> tuple[str, str]:
    for extractor in (extract_with_trafilatura, extract_with_readability):
        text, method = (
            extractor(html)
            if extractor is not extract_with_readability
            else extractor(html)
        )
        if text:
            return text, method
