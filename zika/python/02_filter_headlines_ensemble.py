...
def row_as_series(df: pd.DataFrame, idx: Any) -> pd.Series:
    return cast(pd.Series, cast(Any, df).loc[idx])
...
def reconcile_ensemble_outputs(
    ...
    for idx in df.index.tolist():
        ...
        set_frame_value(df, idx, "rationale_similarity", sim_val)
        set_frame_value(df, idx, "rationale_similarity_method", sim_method)
        set_frame_value(df, idx, "final_action", final_action)
        set_frame_value(df, idx, "ensemble_label", ens_label)
        set_frame_value(df, idx, "processed_at", pd.Timestamp.now("UTC").isoformat())
        if changed:
            updated += 1
...
def write_openai_batch_requests(
    ...
    with request_path.open("w", encoding="utf-8") as handle:
        for idx in indices:
            row = row_as_series(base_df, idx)
            payload = build_openai_batch_request(
                record_id=str(row["record_id"]),
                system_prompt=system_prompt,
                user_prompt=build_prompt(
                    row_text(row, "rss_title"),
                    row_text(row, "source_hint"),
                    row_text(row, "rss_pubdate"),
                ),
...
def apply_openai_batch_results(
    ...
    for raw_line in output_text.splitlines():
        ...
        if status_code >= 400:
            set_frame_value(base_df, idx, "openai_raw_response", raw_body)
            set_frame_value(base_df, idx, "openai_error", f"http_{status_code}")
            errors += 1
            continue
        ...
        if parsed["parse_error"]:
            set_frame_value(base_df, idx, "openai_label", "")
            set_frame_value(base_df, idx, "openai_confidence", np.nan)
            set_frame_value(base_df, idx, "openai_rationale", "")
            set_frame_value(
                base_df, idx, "openai_error", f"parse_error:{parsed['parse_error']}"
            )
            errors += 1
            continue

        set_frame_value(base_df, idx, "openai_model", openai_model)
        set_frame_value(base_df, idx, "openai_label", parsed["label"])
        set_frame_value(base_df, idx, "openai_confidence", parsed["confidence"])
        set_frame_value(base_df, idx, "openai_rationale", parsed["rationale"])
        set_frame_value(base_df, idx, "openai_error", "")
        applied += 1
    ...
    for raw_line in error_text.splitlines():
        ...
        set_frame_value(
            base_df, idx, "openai_raw_response", json.dumps(payload, ensure_ascii=False)
        )
        set_frame_value(base_df, idx, "openai_label", "")
        set_frame_value(base_df, idx, "openai_confidence", np.nan)
        set_frame_value(base_df, idx, "openai_rationale", "")
...
def refresh_validation_assets(
    ...
    if not labeled_val.empty:
        ...
        y_true = np.asarray(labeled_val["gold_label"].tolist(), dtype=object)
        for sys_name, y_pred in [
            ("openai", np.asarray(labeled_val["openai_label"].tolist(), dtype=object)),
            ("local", np.asarray(labeled_val["local_label"].tolist(), dtype=object)),
            (
                "ensemble",
                np.asarray(labeled_val["ensemble_label"].tolist(), dtype=object),
            ),
        ]:
...
def main() -> None:
    ...
    if openai_pending_indices and bool(openai_cfg.get("use_batch", False)):
        ...
        for i, idx in enumerate(openai_pending_indices):
            ...
            try:
                ...
                set_frame_value(base_df, idx, "openai_error", str(exc))
                base_df, _ = sanitize_scored_frame(base_df)
                checkpoint_and_stop(
            ...
            if o_parsed["parse_error"]:
                set_frame_value(base_df, idx, "openai_raw_response", o_raw or o_content)
                set_frame_value(
                    base_df,
                    idx,
                    "openai_error",
            ...
            set_frame_value(base_df, idx, "openai_model", openai_cfg.get("model", ""))
            set_frame_value(base_df, idx, "openai_raw_response", o_raw)
            set_frame_value(base_df, idx, "openai_label", o_parsed["label"])
            set_frame_value(base_df, idx, "openai_confidence", o_parsed["confidence"])
            set_frame_value(base_df, idx, "openai_rationale", o_parsed["rationale"])
            set_frame_value(base_df, idx, "openai_error", "")
    ...
    for i, idx in enumerate(local_pending_indices):
        ...
        try:
            ...
            set_frame_value(base_df, idx, "local_error", str(exc))
            base_df, _ = sanitize_scored_frame(base_df)
            checkpoint_and_stop(
        ...
        if l_parsed["parse_error"]:
            set_frame_value(base_df, idx, "local_raw_response", l_raw or l_content)
            set_frame_value(
                base_df, idx, "local_error", f"parse_error:{l_parsed['parse_error']}"
            )
            base_df, _ = sanitize_scored_frame(base_df)
            checkpoint_and_stop(
        ...
        set_frame_value(base_df, idx, "local_model", local_cfg.get("model", ""))
        set_frame_value(base_df, idx, "local_raw_response", l_raw)
        set_frame_value(base_df, idx, "local_label", l_parsed["label"])
